...
- **The baselines are the pre-switch models.** Each suite's `BASELINE` arm is a constant (gpt-5-nano, gpt-5-mini, gpt-5.2), never read from `config`. Since phase 2 (2026-09-24) production runs the GPT-6 candidates, so a comparison for a future model change should make today's defaults (gpt-6-luna, gpt-6-sol) its baselines.
...
| `--steps` | Comma list of `preassess, assess, dedup, rating-set, social-post, selection` (default: the three checks). `rating-set` and the ship checks run only when named; `rating-set` refuses `--limit`. |
...
- **Ship checks** (`shipChecks.ts`, rules in `shipRules.ts`) cover the other prompt changes that ship with the phase-2 switch, on the whole cached sample whatever `--half` says. `social-post`: Luna at `--effort` drafts every cached Bluesky and Mastodon post; every draft must name the story's main actor or a key number (`storyAnchors.ts`: a capitalised word the story uses mid-sentence, an acronym other than topic ones like AI or HIV, or a number the story states), stay within the limit and state no number the story lacks. Names the story never mentions are listed for the embellishment spot check but do not fail it; the report lists every draft for that read. The heuristic misses a number in words ("a quarter" for 25%) and a story whose only actor is a group ("developers"), so read the drafts it flags before calling a miss. `selection`: gpt-6-sol@medium on every cached selection group with publication dates; the gate is phase 1's (exactly N valid IDs in every group, no declines). Fixtures cached before the date field show each candidate's group crawl day. One candidate per group, the first one production selected, is re-dated to 2023-06-01 as a stale probe; the report compares how often the model keeps it with how often it picked the same stories in phase 1 without dates, read free from the ledger through the frozen phase-1 prompt (`suites/selectPhase1Prompt.ts`; never edit it).
...
| `server/src/scripts/eval/shipChecks.ts` | The ship-check steps (`social-post`, `selection`) |
| `server/src/scripts/eval/suites/selectPhase1Prompt.ts` | Frozen phase-1 selection prompt, for reading phase 1's picks from the ledger |
