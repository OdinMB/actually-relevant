...
- **fetched → pre_analyzed**: Pre-assess job batch-screens stories (~10 per batch) with the medium model, which assigns each story's issue, a 1-10 screening rating and an emotion tag in one call.
...
- **selected → published**: Publish job or manual admin action. The `publish_stories` job publishes all `selected` stories, sets `datePublished` if not already set, and generates a URL slug if the story doesn't have one yet. Embeddings must already exist from the assessment step; `ensureEmbedding()` acts as a safety net for edge cases. Bulk publish (`bulkUpdateStatus`) processes the ids in **bounded chunks** of `config.publish.chunkSize` (default 100, env `PUBLISH_CHUNK_SIZE`) via the `publishChunk` helper. Each chunk runs its own slug-locked interactive transaction and assigns that chunk's new slugs in **one** batched `UPDATE … FROM (VALUES …)` statement (a per-story update loop here previously exceeded Prisma's 5s transaction timeout once the `selected` backlog grew — P2028). Chunking bounds the per-chunk embedding fetch and the transaction so a large backlog publishes over several passes — within Render's memory limit and the transaction timeout — rather than one unbounded pass (which previously OOMed the instance). Each chunk **commits independently**: a failure on a later chunk leaves earlier chunks published, and the next run resumes from the remaining `selected` stories. The transaction timeout is configurable via `DB_TRANSACTION_TIMEOUT_MS` (default 15000ms; see `config.database.transactionTimeoutMs`). Slug generation is a read-modify-write over the global slug namespace, so **every** path that auto-assigns a slug runs through `withSlugLock` — a transaction holding a shared `pg_advisory_xact_lock` that spans both the existing-slug read and the write. **The lock is acquired with `$executeRaw`, not `$queryRaw`**: `pg_advisory_xact_lock()` returns Postgres `void`, and `$queryRaw` tries to deserialize the result column and throws P2010 ("Failed to deserialize column of type 'void'"); `$executeRaw` runs the statement (taking the lock) and deserializes nothing. `withSlugLock` covers bulk publish *and* single-story publish (`publishStory`, `updateStoryStatus`, and the publish branch of `updateStory`); without it two concurrent publishers could pick the same slug for different stories and one would fail with a unique-violation (P2002). The lock is always acquired before any row write (no advisory⇄row lock cycle) and auto-releases on commit.
...
| `publish_stories` | Configurable | Publishes all `selected` stories (in bounded chunks of `config.publish.chunkSize`), sets `datePublished` if not already set |
